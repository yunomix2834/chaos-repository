...
public final class CommandValidator {
    ...
    public static void validateScale(String target, int replicas) {
        if (replicas <= 0) {
            throw new IllegalArgumentException("SCALE replicas must be > 0");
        }
        if (target == null || target.isBlank()) {
            throw new IllegalArgumentException("SCALE target is blank");
        }
        ...
        if (!ok) {
            throw new IllegalArgumentException(
                    "SCALE target format invalid: " + target +
    ...
    public static void validateKillPods(String selector, int percent) {
        if (selector == null || selector.isBlank()) {
            throw new IllegalArgumentException("KILL_PODS selector is blank");
        }
        if (percent <= 0 || percent > 100) {
            throw new IllegalArgumentException(
                    "KILL_PODS percent must be 1..100");
        }
    }
}